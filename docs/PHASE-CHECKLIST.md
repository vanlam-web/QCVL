...
| Current product/inventory/POS direction | Đang hoàn tất Hàng hóa → Kiểm kho hàng thường → Cuộn/tấm/khui object-level → POS trừ kho thật; normal/combo checkout đã có `sale_deduction`, roll/sheet POS object-level deduction còn pending |
...
| Product/Inventory/POS completion | Đang mở | Theo dõi ở `COORD-2026-07-07-PRODUCT-INVENTORY-POS`; phần còn lại chính là POS roll/sheet object-level deduction và drift `/pos/cart/validate` |
