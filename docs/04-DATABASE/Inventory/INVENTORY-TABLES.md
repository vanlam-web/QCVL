...
Lưu phần tồn import từ KiotViet để đối chiếu với tồn QCVL sau này.
...
- Không dùng tồn import như tồn chính, kể cả với hàng `normal`.
- Không tự giảm `remaining_qty` để biến tồn KV thành tồn vận hành. Nếu cần theo dõi chênh lệch, dùng báo cáo đối chiếu giữa `inventory_provisional_balances` và tồn tính từ `stock_movements`.
...
| `provisional_balance_id` | `uuid` | ✅ | FK → `public.inventory_provisional_balances.id` nếu cần lưu tham chiếu đối chiếu; không dùng để sinh tồn chính thức |
...
- `stocktakes.source_creator_name` lưu raw `Người tạo` từ KiotViet nếu file có cột này; đây là dữ liệu audit/mapping, không phải nguồn người tạo thứ hai trong UI.
- `stocktakes.created_by` có thể null với dữ liệu import lịch sử.
- Nếu `source_creator_name` khớp duy nhất với tài khoản QCVL theo `users.username` (có bỏ hậu tố `{DEL}`), import ghi `stocktakes.created_by` bằng user đó để UI/filter dùng một nguồn chuẩn là tài khoản. Không map theo display name, phone hoặc email. Danh sách phải join/hydrate từ `users` hiện tại thay vì dùng snapshot tên lúc import.
