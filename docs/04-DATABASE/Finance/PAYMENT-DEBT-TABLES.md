...
| `customer_id` | `uuid` | ✅ | FK → `public.customers.id`; phiếu từ POS khách lẻ dùng `KH000001`, không tạo bucket null |
...
- Nếu thu từ hóa đơn khách lẻ, `customer_id` là khách mặc định `KH000001`.
...
| `id` | `uuid` | ❌ | Khóa chính |
| `organization_id` | `uuid` | ❌ | FK → `public.organizations.id` |
| `customer_id` | `uuid` | ✅ | FK → `public.customers.id`; khách lẻ nợ dùng `KH000001`, không dùng bucket null |
...
| `balance_after_customer` | `numeric(12,0)` | ✅ | Tổng nợ khách sau giao dịch; với khách lẻ là tổng nợ của `KH000001` |
...
| `retail_debt_note` | `text` | ✅ | Ghi chú nhận diện nếu là khách lẻ nợ trên `KH000001` |
...
- Runtime POS không tạo công nợ khách lẻ với `customer_id` null. Nếu hóa đơn khách lẻ còn nợ, `customer_id` là `KH000001`; `retail_debt_note` dùng để nhận diện người nợ.
- `balance_after_customer` bắt buộc với mọi giao dịch runtime có khách, gồm `KH000001`.
