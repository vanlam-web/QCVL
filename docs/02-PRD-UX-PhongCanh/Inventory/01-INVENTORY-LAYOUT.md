...
| BOM/Vật tư cấu thành | Nhập/sửa định mức vật tư cho combo hoặc sản phẩm có BOM; combo có thể hiển thị tóm tắt vật tư ở tab `Thông tin`, nhưng nguồn sửa chính vẫn là tab này để tránh làm tab Thông tin quá nặng. Nếu có BOM import từ KiotViet, tab này hiện `BOM KiotViet`, số vật tư và trạng thái `Đang dùng khi bán` |
