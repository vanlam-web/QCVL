...
- Shared time: `src/lib/date-format.ts` la helper chung cho hien thi `dd/MM/yyyy HH:mm`; cac presenter/page moi khong tu tao `Intl.DateTimeFormat` rieng neu khong co ly do ro trong doc. Gio QCVL/KiotViet hien tren UI la gio lam viec da ghi trong DB/API; khong cong them timezone trong presenter/page. Cung mot chung tu phat sinh hoa don va so quy phai hien cung thoi gian.
