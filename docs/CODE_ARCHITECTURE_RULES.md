...
9a. O ngay-gio trong form/dialog phai dung `ManagementDateTimeInput` khi layout cho phep; neu can layout rieng nhu POS/customer debt thi van phai dung helper chung `src/components/ui-shell/management-date-time-picker.ts` cho calendar grid va danh sach gio. Khong tao lai `Array.from(48)` hay ham calendar rieng trong page.
...
- Shared date-time picker: `src/components/ui-shell/management-date-time-picker.ts` gom calendar grid va danh sach gio dung chung cho `ManagementDateTimeInput`, POS checkout, customer debt payment/adjustment. Khong copy ham calendar/time options vao page moi.
