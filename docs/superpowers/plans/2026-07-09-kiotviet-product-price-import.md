...
- Superseded after this plan: stock is now written as KiotViet provisional stock; BOM from `Hàng thành phần` is parsed into DB. **SoT** Owner 2026-07-20 = active/usable immediately; **runtime still draft** — `docs/03-BUSINESS-NghiepVu/BOM/README.md` §2. Supplier and expected-out-of-stock remain deferred.
...
- Scope matches Phase 2 only: price import and display. Later work added provisional stock and BOM parse from `Hàng thành phần` in the main product import plan (**SoT** usable immediately per Owner 2026-07-20; **runtime alignment pending** — BOM README §2); supplier and expected-out-of-stock remain deferred.
