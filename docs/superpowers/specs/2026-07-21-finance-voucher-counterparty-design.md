...
Status: **partially implemented (2026-07-21)**. Slice đã làm: nút `Tạo mới` trên form phiếu thu/chi mở tạo nhanh **Khách hàng**, **Nhà cung cấp**, **Nhân viên** và **Đối tác giao hàng** rồi điền lại tên vào phiếu; các nhóm master data phải lưu `counterparty_id` + snapshot tên/số điện thoại. Chưa làm: màn quản lý riêng đầy đủ cho `Nhân viên` và `Đối tác giao hàng`, map loại thu/chi → nhóm đối tượng ở API, combobox bắt buộc chọn bản ghi thay cho datalist.
...
- Nhân viên lấy từ danh sách nhân viên riêng, không trộn với tài khoản đăng nhập.
...
| `Nhân viên` | Employees API (`public.employees.id`) | Không; phải chọn nhân viên có sẵn hoặc bấm `Tạo mới`. |
...
| `Nhân viên` | Bảng `employees` | Chọn nhân viên cũ hoặc tạo nhanh nhân viên mới; không lưu text tạm. | Giai đoạn đầu quản lý nhẹ trong combobox/tạo nhanh; sau đó có màn quản lý riêng nếu phát sinh nhu cầu. |
| `Đối tác giao hàng` | Bảng `delivery_partners` | Chọn đối tác cũ hoặc tạo nhanh đối tác mới; không lưu text tạm. | Giai đoạn đầu quản lý nhẹ trong combobox/tạo nhanh; sau đó có màn quản lý riêng nếu phát sinh nhu cầu. |
| `Khác` | Không có master data | Chỉ lưu text trên voucher, không đưa vào danh sách gợi ý chung. | Không có màn quản lý riêng. |

Quy tắc thêm nhanh:

- Nút `Tạo mới` chỉ hiện khi nhóm đối tượng có master data thật: khách hàng, nhà cung cấp, nhân viên, đối tác giao hàng.
- Với khách hàng/NCC, `Tạo mới` mở form tạo nhanh đúng module và sau khi lưu quay lại phiếu với id đã chọn.
- Với nhân viên và đối tác giao hàng, form tạo nhanh tối thiểu có tên, mã tự sinh nếu bỏ trống, số điện thoại.
