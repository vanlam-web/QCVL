...
Status: **partially implemented (2026-07-21)**. Slice đã làm: nút `Tạo mới` trên form phiếu thu/chi mở tạo nhanh **Khách hàng** / **Nhà cung cấp** rồi điền lại tên vào phiếu; nhóm **Nhân viên** tìm từ `/users?status=active`, bắt buộc chọn tài khoản có sẵn và lưu `counterparty_id` + snapshot tên/số điện thoại. Chưa làm: bắt buộc `counterparty_id` cho toàn bộ khách hàng/NCC/đối tác giao hàng, đối tác giao hàng, map loại thu/chi → nhóm đối tượng, combobox bắt buộc chọn bản ghi thay cho datalist.
...
| `Nhân viên` | Users API (`public.users.id`) | Không; phải chọn nhân viên/tài khoản có sẵn. |
