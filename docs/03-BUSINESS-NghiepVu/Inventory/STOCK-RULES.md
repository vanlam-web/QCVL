...
### BR-INV-02B: POS được bán trước, đối soát vật tư vật lý sau

QC-OMS ưu tiên không làm nghẽn bán hàng tại POS.

Với hàng `roll` và `sheet`, checkout không bị chặn chỉ vì nhân viên chưa biết cuộn/tấm vật lý nào sẽ được dùng. Hệ thống vẫn được phép tạo hóa đơn, ghi snapshot dòng hàng và ghi cảnh báo cần đối soát vật tư sau.

Có hai tình huống vận hành:

| Tình huống | Quy tắc |
|---|---|
| In/sản xuất trước, nhập bill sau | Nếu đã biết cuộn/tấm/khổ thực tế, nhân viên có thể gán object vật lý ngay khi lập bill hoặc sau khi lập bill. |
| Làm bill trước, in/sản xuất sau | Hóa đơn được lưu trước. Khi máy sản xuất gửi thông tin khổ/cuộn/file sau, hệ thống dùng dữ liệu đó để đề xuất gán object phù hợp cho dòng bill. |

Quy tắc gán object:

- Một dòng bill chỉ gán tối đa một object vật lý tại một thời điểm.
- Một bill có thể có nhiều dòng, nên có thể gán nhiều cuộn/tấm khác nhau qua các dòng khác nhau.
- Nếu một sản phẩm cần dùng nhiều cuộn/tấm, nhân viên tách thành nhiều dòng bill.
- Báo giá không giữ trước object vật lý; object chỉ gán khi đã bán hoặc khi đối soát sản xuất.
- Hóa đơn cũ trước khi có rule gán object không bắt buộc backfill. Chỉ đối soát lại khi có dữ liệu thực tế và Owner/nhân viên cần.

Trạng thái dòng hàng nên phân biệt được:

| Trạng thái | Ý nghĩa |
|---|---|
| `not_required` | Dòng không cần object vật lý, ví dụ hàng `normal` hoặc dịch vụ. |
| `unassigned` | Dòng cần hoặc có thể gán object nhưng chưa biết cuộn/tấm thực tế. |
| `suggested` | Hệ thống có đề xuất object dựa trên khổ/kích thước/máy sản xuất, chờ nhân viên xác nhận. |
| `assigned` | Dòng đã gán object vật lý thực tế. |
| `overridden` | Nhân viên chọn object khác đề xuất; hệ thống lưu lý do nếu có. |

### BR-INV-03: Không dùng dữ liệu máy sản xuất để tự trừ kho MVP

Dữ liệu máy sản xuất không tự sinh bút toán kho hoặc tự đổi object đã gán trong MVP.

Nếu máy sản xuất chạy khác với bill/đơn, phần lệch chỉ thể hiện trong báo cáo/queue đối soát. Hệ thống được phép tạo đề xuất gán object phù hợp dựa trên khổ/kích thước/thời điểm sản xuất, nhưng nhân viên vẫn là người xác nhận trước khi dữ liệu kho vật lý được coi là đã đối soát.
...
Khi bán hàng mà tồn không đủ, kể cả tồn vật lý theo cuộn/tấm không đủ hoặc chưa chọn được cuộn/tấm, hệ thống cảnh báo nhưng vẫn cho bán tiếp.

Hệ quả:

- stock movement có thể làm tồn kho âm
- tồn vật lý theo cuộn/tấm có thể tạm âm hoặc có trạng thái chưa đối soát
...
Khi xuất vật tư dạng cuộn, nhân viên là người xác nhận cuộn bị trừ/đối soát. Việc chọn cuộn không bắt buộc phải hoàn tất trước checkout POS.
...
Nếu checkout đã tạo hóa đơn nhưng chưa biết cuộn thực tế, hệ thống ghi dòng ở trạng thái chưa đối soát. Khi máy sản xuất gửi thông tin khổ/cuộn hoặc khi nhân viên cập nhật sau, hệ thống tạo đề xuất dựa trên thông tin này để nhân viên xác nhận.
...
- Bán hàng `roll`/`sheet` không bị chặn khi chưa chọn object vật lý.
- Dòng `roll`/`sheet` chưa chọn object được đánh dấu cần đối soát sau.
- Một dòng bill chỉ gán tối đa một object vật lý; bill có thể nhiều dòng và nhiều object.
- Báo giá không giữ trước object vật lý.
