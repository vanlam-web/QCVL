...
Backend vẫn deep-scan khi chốt hóa đơn nếu BOM nhiều cấp đã có đủ DB/API. Combo phẳng cấp 1 với BOM `active` (kể cả import KiotViet) trừ thành phần ngay theo BR-BOM-08.
...
- Dữ liệu BOM KV cũ còn `draft` được migrate/promote sang `active` (hoặc archive + import lại) trước khi coi runtime khớp SoT.
- UI/API không còn copy “BOM nháp / cần rà soát” cho BOM import từ KiotViet; field `draft_bom` nếu còn chỉ là tên tương thích.