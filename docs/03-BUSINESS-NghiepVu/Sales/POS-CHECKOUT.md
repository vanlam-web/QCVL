...
Nếu chưa chọn khách hàng cụ thể nhưng hóa đơn còn nợ, hệ thống vẫn cho phép hoàn tất thanh toán dưới dạng **Khách lẻ nợ** thuộc khách mặc định `khachle - Khách lẻ`.
...
Khoản nợ khách lẻ không tạo bucket `customer_id = null`. Nếu sau này chuyển hóa đơn/khoản nợ từ `khachle` sang khách cụ thể, cần spec riêng vì đây là thao tác đổi chủ công nợ/chứng từ.
...
| **Loại 3 (Combo/BOM)** | Trừ từng vật tư thành phần theo BOM. **Không** trừ tồn theo mã combo. BOM chuẩn từ danh mục (mục tiêu: active sau import KV) hoặc BOM dòng POS khi phase đó đã làm |

Giá bán combo độc lập với tổng giá vật tư. **Owner 2026-07-20** + SoT: [BOM README](../BOM/README.md).

**Hướng dài (chưa runtime đầy đủ):** `Không lưu — Chỉ trừ kho` / `Lưu Combo mới` trên dòng POS.

**Hiện trạng code (2026-07-20):** Postgres POS còn có thể trừ cả mã combo — chưa nghiệm thu BR-CHK-04 cho combo cho đến khi sửa runtime (BOM README mục 2).
...
Nếu còn nợ nhưng chưa chọn khách, phần còn nợ được lưu vào `khachle - Khách lẻ` dưới dạng **Khách lẻ nợ** kèm ghi chú nợ bắt buộc.
