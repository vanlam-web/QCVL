...
- API dev bắt buộc chạy bằng `npm run api:dev`. Script này nạp PostgreSQL NAS từ `\\192.168.1.188\docker\QCVL\.env`, dùng host Tailscale `100.84.228.125:55433`, rồi chạy `tsx watch server/index.ts`. Health phải trả `persistence: "postgres"` khi cần `3202` chung dữ liệu với `3200`; nếu không đọc được env/DB NAS thì script phải fail rõ, không được âm thầm chạy memory. Không chạy tay `tsx server/index.ts` vì sẽ giữ backend cũ và route mới có thể trả 404.
