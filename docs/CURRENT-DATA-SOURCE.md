...
- API dev bat buoc chay bang `npm run api:dev`. Script nay nap PostgreSQL NAS tu `\\100.84.228.125\docker\QCVL\.env`, dung host Tailscale `100.84.228.125:55433`, roi chay `tsx watch server/index.ts`. Health phai tra `persistence: "postgres"` khi can `3202` chung du lieu voi `3200`; neu khong doc duoc env/DB NAS thi script phai fail ro, khong duoc am tham chay memory. Khong chay tay `tsx server/index.ts` vi se giu backend cu va route moi co the tra 404.
...
- Finance cashbook detail phai phan biet 2 thoi diem: `Thời gian:` o dau phieu la gio ghi sổ/thu tien cua cashbook; cot `Thời gian` trong bang `Chứng từ liên kết` phai show gio hoa don/phiếu goc neu co, khong lay gio cashbook. Neu allocation chua co `order_created_at` thi hydrate tu `orders.created_at`.
...
- Ngay `2026-07-21`: local `3202` `/receipts/new` da sua chon khổ cho hang nhap dang ma don vi nhu `B260`. Sau khi chon `B260`, UI tai them san pham cung ten/cung `inventory_shape` de dropdown `ĐVT` co cac khổ lien quan; chon `Khổ 100` phai doi dong sang ma `B100`, gia nhap cuoi cua `B100`, va submit `product_id` cua `B100`. Cot `ĐVT` trong workspace nhap hang duoc noi rong de khong xo cac o nhap canh nhau. Local verified tren browser `3202`: `B260` -> `Khổ 100` hien `B100`, gia `785 604`.
- Ngay `2026-07-21`: local `3202` `/receipts/new` NCC search khong con chi loc cache ban dau. Lookup ban dau nap toi da 100 NCC active, va khi go trong o `Nha cung cap` se goi `/api/v1/suppliers?q=...&status=active` nhu trang `Nha cung cap`. Browser verified: tim `cpds` hien `Chiến Phượng Diên Sanh`, chon xong hien selected supplier block.
- Ngay `2026-07-21`: customer debt canonical phai clamp debt row ve hoa don that: neu `customer_debt_entries.remaining_debt` lon hon `orders.debt_amount`, cac API tong/detail/thu no chi duoc dung `least(remaining_debt, debt_amount)`. DB NAS da sua 1 dong ban `HD011282` cua khach `HLo` tu `5 115 000` ve `115 000`; audit sau sua khong con dong open `remaining_debt > orders.debt_amount`. Khong duoc neo UI vao tong hien tai roi tinh nguoc de ep khop. Lich su cong no chi hien so du luy ke khi co moc that: uu tien `customer_debt_adjustments` (`CB...`, can bang/chiet khau/dieu chinh cong no), neu khong co thi dung snapshot KiotViet `customer_snapshots.data.total_debt_amount` tai `last_transaction_at` nhu dong mo so `Số dư KiotViet`. Sau moc that: hoa don tang no, phieu thu/thu no giam no, dong dieu chinh that tang/giam no theo `amount_delta`. Neu khong co moc that, cot so du luy ke phai de `-`, khong tu bia so.
