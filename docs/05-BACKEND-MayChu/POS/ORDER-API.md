...
- Hóa đơn/báo giá tạo trực tiếp từ POS (`HD-POS-*`, `BG-POS-*`) dùng `created_at` tại thời điểm backend nhận checkout/quote. UI hiển thị theo giờ local vận hành Việt Nam; không lấy giờ tạo tab nháp làm giờ hóa đơn.
...
- Náº¿u Frontend khÃ´ng gá»­i `customer_id`, Backend resolve vá» khÃ¡ch máº·c Ä‘á»‹nh `khachle - KhÃ¡ch láº»` trÆ°á»›c khi lÆ°u.
...
`source_quote_id`, `customer_id`, `price_list_id`, `bank_account_id` Ä‘Æ°á»£c phÃ©p null trong input theo nghiá»‡p vá»¥. Khi ghi hÃ³a Ä‘Æ¡n, `customer_id` null pháº£i Ä‘Æ°á»£c Backend resolve vá» `khachle - KhÃ¡ch láº»`; dá»¯ liá»‡u lÆ°u khÃ´ng dÃ¹ng bucket khÃ¡ch láº» null.
...
- Náº¿u Frontend khÃ´ng gá»­i `customer_id`, Backend resolve vá» `khachle - KhÃ¡ch láº»`.
- Náº¿u khÃ¡ch Ä‘Æ°á»£c resolve lÃ  `khachle` vÃ  hÃ³a Ä‘Æ¡n cÃ²n ná»£, `retail_debt_note` báº¯t buá»™c Ä‘á»ƒ nháº­n diá»‡n ngÆ°á»i ná»£.
...
3. Sinh mÃ£ `HD...`.
4. Resolve khÃ¡ch hÃ ng: náº¿u input khÃ´ng cÃ³ khÃ¡ch, dÃ¹ng `khachle - KhÃ¡ch láº»`.
