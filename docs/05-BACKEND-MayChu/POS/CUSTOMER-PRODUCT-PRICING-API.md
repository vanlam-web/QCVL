...
`Hàng thành phần` import từ KiotViet được parse theo dạng `Mã:Định mức|Mã:Định mức` và lưu thành BOM dùng ngay (`product_boms.status = active`, Owner 2026-07-20). Khi bán combo, backend trừ kho theo thành phần; không trừ tồn theo mã combo. Không còn bắt buộc duyệt/kích hoạt lại sau import. `Dự kiến hết hàng` vẫn làm sau bằng luồng riêng.
