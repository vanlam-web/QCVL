...
`Hàng thành phần` import từ KiotViet được parse theo dạng `Mã:Định mức|Mã:Định mức` và lưu thành BOM `active` (`Trusted for stock deduction`), dùng ngay khi bán. POS trừ tồn thành phần theo BOM `draft`|`active`; không trừ tồn mã combo (`track_inventory = false` / `product_kind = combo`). `Dự kiến hết hàng` vẫn làm sau bằng luồng riêng.
...
- Xóa dữ liệu phụ KiotViet: tồn tạm `inventory_provisional_balances.source_type = kiotviet_import`, BOM import KiotViet, giá import liên quan.
