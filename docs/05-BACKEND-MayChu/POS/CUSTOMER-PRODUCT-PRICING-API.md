...
`Hàng thành phần` import từ KiotViet được parse theo dạng `Mã:Định mức|Mã:Định mức`. **SoT Owner 2026-07-20:** lưu `product_boms.status = active`, bán combo trừ thành phần (không trừ mã combo), không duyệt/kích hoạt lại. **Runtime 2026-07-20 chưa khớp** (vẫn `draft`; Postgres POS có thể trừ cả mã combo) — xem `docs/03-BUSINESS-NghiepVu/BOM/README.md` mục 2. `Dự kiến hết hàng` vẫn làm sau bằng luồng riêng.
