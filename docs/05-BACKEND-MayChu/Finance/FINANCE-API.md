...
- in phiếu thu/chi từ dữ liệu detail đã lưu
...
Danh sách hóa đơn còn nợ của khách mặc định `khachle - Khách lẻ`.
...
| `counterparty_type` | `string` | Không | `customer`, `supplier`, `employee`, `delivery_partner`, `other`, `none` |
...
### `GET /finance/delivery-partners`

Danh sách đối tác giao hàng dùng cho combobox người nộp/nhận trong phiếu thu/chi.

**Permission:** `perm.manage_finance`

**Query:** `search`, `is_active`, `page`, `page_size`.
...
### `POST /finance/delivery-partners`

Tạo nhanh đối tác giao hàng trước khi lưu phiếu thu/chi.
...
- `name` bắt buộc.
- Nếu có `phone`, chuẩn hóa và chống trùng theo `(organization_id, normalized_phone)`.
- Nếu không có `phone`, chống trùng theo `(organization_id, normalized_name)`.
- Nếu trùng, trả về bản ghi đã có thay vì tạo bản ghi mới.
...
- Chỉ `counterparty_type = 'other'` được nhập text tự do và lưu `counterparty_name` không kèm `counterparty_id`.
- Với `customer`, `supplier`, `employee`, `delivery_partner`, `counterparty_id` bắt buộc và phải cùng organization.
- Với `employee`, `counterparty_id` là `public.profiles.id`.
- Với `delivery_partner`, nếu tên mới chưa có thì client phải gọi `POST /finance/delivery-partners` trước, sau đó gửi `counterparty_id` vào voucher.
- Backend phải snapshot `counterparty_name` và `counterparty_phone` từ master data lúc tạo/sửa phiếu; không phụ thuộc tên master data về sau.
...
`Lưu & In` là hành vi client: gọi endpoint tạo/sửa trước, sau đó dùng response hoặc `GET /finance/cashbook-vouchers/{id}/print` để in. Lỗi in không rollback phiếu đã lưu.
...
Input revise dùng cùng field với tạo phiếu, cộng thêm `revision_reason`. Backend phải snapshot lại counterparty/tài khoản tại thời điểm tạo bản sửa mới.
...
Trả dữ liệu in phiếu thu/chi đã lưu.

**Permission:** `perm.view_shift_report` hoặc `perm.manage_finance`

Response gồm tối thiểu:

- mã phiếu, trạng thái, thời gian, người tạo/người thu/chi
- loại thu/chi, phương thức TT, tài khoản
- đối tượng nộp/nhận, số điện thoại nếu có
- số tiền, ghi chú, hạch toán
- chứng từ liên quan/phân bổ nếu có

Endpoint này chỉ trả dữ liệu. Client quyết định in bằng trình duyệt, máy in hệ điều hành, hoặc export file ở slice sau.
