...
it('opens supplier detail in view mode before editing and saves inactive status', async () => {
  ...
  expect(within(historyTable).queryByRole('columnheader', { name: 'Đã trả' })).not.toBeInTheDocument()
  ...
  expect(within(debtTable).queryByRole('columnheader', { name: 'Đã trả' })).not.toBeInTheDocument()
