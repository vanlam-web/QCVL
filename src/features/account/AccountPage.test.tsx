...
it('renders account management cards from the current user profile', () => {
  ...
  expect(within(userCard).getAllByText('Chưa có', { selector: 'dd' })).toHaveLength(3)
  ...
  expect(within(userCard).getByText('Chưa có ghi chú')).toBeInTheDocument()
...
it('opens the account edit dialog with real profile values and empty missing fields', async () => {
  ...
  expect(within(dialog).getByLabelText('Điện thoại')).toHaveAttribute('placeholder', 'Chưa có')
  ...
  expect(within(dialog).getByLabelText('Sinh nhật')).toHaveAttribute('placeholder', 'Chưa có')
  expect(within(dialog).getByLabelText('Khu vực')).toHaveAttribute('placeholder', 'Chưa có')
  expect(within(dialog).getByLabelText('Phường/Xã')).toHaveAttribute('placeholder', 'Chưa có')
  expect(within(dialog).getByLabelText('Địa chỉ')).toHaveAttribute('placeholder', 'Chưa có')
  expect(within(dialog).getByLabelText('Ghi chú')).toHaveAttribute('placeholder', 'Chưa có')
